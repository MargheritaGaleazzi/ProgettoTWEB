...
class Catalogo {
    ...
    public function getEventiFiltrati($luogo = null, $societa = null, $data = null, $descrizione = null) {
        ...
        return Evento::where('informazioni', 'LIKE', '%' . $descrizione . '%')
                        ...
                        ->where('societa_organizzatrice', 'LIKE', '%' . $societa . '%')
                        ->paginate(4);
        /* if (is_null($luogo) && is_null($societa) && $data == '01-0000' && is_null($descrizione)) {
          return Evento::paginate(4);
          } elseif (is_null($societa) && $data == '01-0000' && is_null($descrizione)) {
          return Evento::where('luogo', $luogo)
          ->paginate(4);
          } elseif (is_null($luogo) && is_null($societa) && is_null($descrizione)) {
